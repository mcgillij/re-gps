...
Location = namedtuple("Location", ["timestamp", "latitude", "longitude", "accuracy", "altitude"])
...
def build_location_history(locations):
    location_history = [
        Location(
            get_timestamp(l["timestamp"]),
            l["latitudeE7"],
            l["longitudeE7"],
            l["accuracy"],
            l["altitude"] if l.get("altitude") else 0
        ) for l in locations if l.get("latitudeE7")]
    # sort by timestamp
